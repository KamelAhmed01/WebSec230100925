<x-layout>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center mb-6">
            ...
            <div class="flex items-center space-x-2">
                ...
                <div class="bg-blue-50 py-1.5 px-3 rounded-full text-sm font-medium flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5 text-blue-600" viewBox="0 0 20 20" fill="currentColor">
                        <path d="M4 4a2 2 0 00-2 2v1h16V6a2 2 0 00-2-2H4z" />
                        <path fill-rule="evenodd" d="M18 9H2v5a2 2 0 002 2h12a2 2 0 002-2V9zM4 13a1 1 0 011-1h1a1 1 0 110 2H5a1 1 0 01-1-1zm5-1a1 1 0 100 2h1a1 1 0 100-2H9z" clip-rule="evenodd" />
                    </svg>
                    <span class="text-blue-800">Your Balance: <span class="font-bold">${{ number_format(auth()->user()->credit->amount ?? 0, 2) }}</span></span>
                </div>
                @endif
                ...
                @can('add_products')
                <a href="{{ route('products_edit') }}" class="bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 transition duration-300 flex items-center">
        ...
        <div class="mb-5 bg-gray-50 p-4 rounded-md">
                    ...
                    <div class="flex flex-wrap items-end gap-4">
                        ...
                        <div class="flex-grow min-w-[200px]">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div class="flex items-center bg-white px-2 py-1 rounded-full shadow-sm border border-gray-200">
                                    <span class="text-gray-500 mr-1">$</span>
                                    <input type="number" id="min_price" name="min_price" value="{{ request('min_price') ?? 0 }}"
                                        class="w-12 border-0 p-0 text-center focus:ring-0"
                                        placeholder="Min">
                                </div>
                                ...
                                <div class="flex items-center bg-white px-2 py-1 rounded-full shadow-sm border border-gray-200">
                                    <span class="text-gray-500 mr-1">$</span>
                                    <input type="number" id="max_price" name="max_price" value="{{ request('max_price') ?? $maxPrice }}"
                                        class="w-12 border-0 p-0 text-center focus:ring-0"
                                        placeholder="Max">
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="inline-flex rounded-md shadow-sm" role="group" aria-label="Sort options">
                                <input type="hidden" name="order_by" id="order_by" value="{{ request('order_by', 'name') }}">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="hidden" name="order_direction" id="order_direction" value="{{ request('order_direction', 'ASC') }}">
                                <button type="button" id="direction-toggle"
                                    ...
                                    <div id="direction-icon" class="transition-transform duration-300 {{ request('order_direction', 'ASC') == 'DESC' ? 'rotate-180' : '' }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-600" viewBox="0 0 20 20" fill="currentColor">
                                            <path d="M3 3a1 1 0 000 2h11a1 1 0 100-2H3zM3 7a1 1 0 000 2h7a1 1 0 100-2H3zM3 11a1 1 0 100 2h4a1 1 0 100-2H3zM15 8a1 1 0 10-2 0v5.586l-1.293-1.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L15 13.586V8z" />
                                        </svg>
                                    </div>
                                    <span id="direction-text">{{ request('order_direction', 'ASC') == 'ASC' ? 'Ascending' : 'Descending' }}</span>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="ml-auto">
                            <div class="flex space-x-2">
                                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors flex items-center gap-2">
                                ...
                                <a href="{{ route('products_list') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300 transition-colors flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div class="mt-4 bg-red-50 border-l-4 border-red-500 p-4 relative" role="alert" id="error-message">
                <div class="flex">
                    ...
                    <button type="button" onclick="document.getElementById('error-message').style.display='none'" class="ml-auto -mx-1.5 -my-1.5 bg-red-50 text-red-500 rounded-lg focus:ring-2 focus:ring-red-400 p-1.5 hover:bg-red-200 inline-flex h-8 w-8">
            ...
            @if (session('success'))
            <div class="mt-4 bg-green-50 border-l-4 border-green-500 p-4 relative" role="alert" id="success-message">
                <div class="flex">
                    <div class="flex-shrink-0">
                    ...
                    <button type="button" onclick="document.getElementById('success-message').style.display='none'" class="ml-auto -mx-1.5 -my-1.5 bg-green-50 text-green-500 rounded-lg focus:ring-2 focus:ring-green-400 p-1.5 hover:bg-green-200 inline-flex h-8 w-8">
                        <span class="sr-only">Dismiss</span>
                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
                        </svg>
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
                @foreach ($products as $product)
                    <div class="bg-white shadow-sm rounded-lg overflow-hidden border border-gray-200 hover:shadow-lg transition-all duration-300 flex flex-col h-full">
                        <!-- Square aspect ratio image container -->
                        <div class="aspect-square bg-white flex items-center justify-center overflow-hidden p-3 border-b border-gray-100">
                        ...
                        <div class="p-4 flex flex-col flex-grow">
                            ...
                            <div class="flex justify-between items-center mb-3">
                                ...
                                @can('make_purchases')
                                    @if($product->stock > 10)
                                    <span class="bg-green-100 text-green-800 text-xs px-1.5 py-0.5 rounded-full whitespace-nowrap">{{ $product->stock }} in stock</span>
                                    @elseif($product->stock > 0)
                                    <span class="bg-red-100 text-red-800 text-xs px-1.5 py-0.5 rounded-full whitespace-nowrap">🔥 {{ $product->stock }} left!</span>
                                    @elseif(!auth()->user()->hasRole('customer'))
                                    <span class="bg-gray-100 text-gray-800 text-xs px-1.5 py-0.5 rounded-full whitespace-nowrap">Out of stock</span>
                                    @endif
                                @else
                                    @if($product->stock > 0)
                                    <span class="bg-green-100 text-green-800 text-xs px-1.5 py-0.5 rounded-full whitespace-nowrap">{{ $product->stock }} in stock</span>
    ...
    @if ($product->stock > 0)
        <div class="mt-auto pt-3 border-t border-gray-100">
            <form action="{{ route('purchases.store', $product->id) }}" method="post" class="purchase-form"
    data-price="{{ $product->price }}"
    data-credit="{{ auth()->user()->credit->amount ?? 0 }}">
                @csrf
                <div class="flex items-center mb-2">
                    <label for="quantity-{{ $product->id }}" class="text-xs text-gray-600 mr-2">Qty:</label>
                    <input
                        type="number"
                    ...
                    <div class="ml-auto text-xs font-medium">
                        <span class="total-price">${{ number_format($product->price, 2) }}</span>
                    </div>
                </div>
                ...
                <div class="credit-warning hidden mb-2 text-xs text-red-600 flex items-center">
                    ...
                    <span>Insufficient credit. You need <span class="shortfall"></span> more.</span>
                </div>

                <button type="submit" class="w-full bg-green-600 hover:bg-green-700 text-white font-medium py-1.5 px-4 rounded transition duration-300 flex items-center justify-center text-sm purchase-button">
    ...
    @else
        <div class="mt-auto pt-3 border-t border-gray-100">
            <div class="bg-red-50 text-red-700 text-xs py-1.5 px-2 rounded flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
@endcan
                            ...
                            @if (auth()->check() && (auth()->user()->can('edit_products') || auth()->user()->can('delete_products')))
                                <div class="flex space-x-1 mt-3 pt-3 border-t border-gray-100">
                                    @can('edit_products')
                                        <a href="{{ route('products_edit', $product->id) }}" class="inline-flex items-center px-2 py-1 bg-indigo-50 text-indigo-700 rounded hover:bg-indigo-100 transition duration-200 flex-1 justify-center text-xs">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    @can('delete_products')
                                        <form action="{{ route('products_delete', $product->id) }}" method="post" class="flex-1" onsubmit="return confirm('Are you sure you want to delete this product?');">
                                            {{ csrf_field() }}
                                            <button type="submit" class="inline-flex items-center px-2 py-1 bg-red-50 text-red-700 rounded hover:bg-red-100 transition duration-200 w-full justify-center text-xs">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
